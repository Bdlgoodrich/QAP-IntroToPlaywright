...
export class CheckoutPage {
    ...
    }  //empty but not default

    // Example seam to navigate to specific steps in the checkout flow
    async visit(step: 'step-one' | 'step-two' | 'complete') {  //utilitzing url framework to 
        await this.page.goto(`/checkout-${step}.html`)
        return this
    }

    async fillYourInfo(firstName: string = "defaultName", lastName: string, postalCode: string) {
